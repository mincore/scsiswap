...
got_data:
	...
	if (sdkp->first_scan || old_capacity != sdkp->capacity) {
		sector_t reserved = scsi_get_reserve_sectors_for_swap(sdp);
		if (reserved && sdkp->capacity > reserved) {
			sdkp->capacity -= reserved;
			sd_printk(KERN_NOTICE, sdkp, 
					"reserved %dGB for swap, new capacity: %llu\n", 
					(int)(reserved/(2*1024*1024)), 
					sdkp->capacity);   
		}
	}
...
#ifdef CONFIG_SCSI_SWAP_BADSECTORS
	/* scsi swap must be initialized before disk add to sysfs */
	scsi_swap_init(&sdp->swap, sdkp->disk, sdkp->capacity);
#endif

	add_disk(gd);

#ifdef CONFIG_SCSI_SWAP_BADSECTORS
	/* scsi swap sysfs should be register after disk add to sysfs */
	scsi_swap_register_sysfs(&sdp->swap);
#endif
...
#ifdef CONFIG_SCSI_SWAP_BADSECTORS
	scsi_swap_unregister_sysfs(&sdkp->device->swap);
#endif
...
#ifdef CONFIG_SCSI_SWAP_BADSECTORS
	scsi_swap_destroy(&sdkp->device->swap);
#endif
